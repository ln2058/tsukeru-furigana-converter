...
async function initSettingsForm() {
  ...
  async function applyFuriganaToPage() {
    ...
    try {
      ...
      const response = await browserApi.tabs.sendMessage(tab.id, { action: 'applyFurigana', settings });
      if (response?.ok) {
  ...
  async function clearFuriganaFromPage() {
    ...
    try {
      await browserApi.tabs.sendMessage(tab.id, { action: 'clearFurigana' });
...
async function ensureContentScript(tabId) {
  ...
  await browserApi.tabs.insertCSS(tabId, { file: 'content.css' });
...
async function initVocabularyTab() {
  ...
  clearBtn.addEventListener('click', async () => {
    ...
    if (confirm(`Clear all ${currentVocabulary.length} vocabulary items?`)) {
      await browserApi.storage.local.set({ vocabulary: [] });
      await loadVocabulary();
    }
  });
}
...
async function loadVocabulary() {
  try {
    const result = await browserApi.storage.local.get(['vocabulary']);
...
async function lookupDefinition(word) {
  ...
  const response = await browserApi.runtime.sendMessage({ action: 'lookupDefinition', word: term });
  if (!response?.success) {
...
async function deleteVocabItem(entryId) {
  try {
    ...
    await browserApi.storage.local.set({ vocabulary: currentVocabulary });
...
async function loadVocabMode() {
  ...
  try {
  const response = await browserApi.tabs.sendMessage(tab.id, { action: 'getPageWords' });
    if (response?.words) {
